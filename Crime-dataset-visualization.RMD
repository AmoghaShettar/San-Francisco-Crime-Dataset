...
df <- read.csv('Police_Department_Incidents_Previous_Year__2016_.csv')
...
plot_robbery <- ggplot(df_robbery_time, aes(x = Hour, y = DayOfWeek, fill = count)) +
              ...
              title = "Number of Robbery in San Francisco over a day") +
...
#Finding the area of each district.
...
#Dataframe constiting area and number of crimes per district 
df_category_area <- df %>%
               group_by(PdDistrict) %>%
               summarize(count = n())
...
names(area_district)[names(area_district) == "df_category_area.count"] <- "Crime_Count"

#Correlation between area and number of crimes
...
Area_t <- c(B, C, I, M, N, P, R, S, TA, T)
df_category_area_T <- df %>%
               group_by(PdDistrict) %>%
               summarize(count = n())
df_category_area_T <- df_category_area_T[-c(1), ]
area_district_T <- data.frame(Area_t, df_category_area_T$count, df_category_area_T$PdDistrict)
names(area_district_T)[names(area_district_T) == "df_category_area_T.count"] <- "Crime_Count"
district_area_tree <- area_district_T %>% select(Area_t, df_category_area_T.PdDistrict)

# TREE MAP area-wise
plot_district <- treemap(district_area_tree,
                index = c("df_category_area_T.PdDistrict","Area_t"),
                vSize = "Area_t",
                type = "index",
                palette = "Blues")
